...
const char kWindowTitle[] = "GC1A_10_チョウテンロウ";

const int kParticleNum = 200;
const int kChargeNum = 60;
const int kSparkNum = 80;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Sparkling* spark[kSparkNum]{};
	for (int i = 0; i < kSparkNum; i++) {
		spark[i] = new Sparkling;
		spark[i]->Initialization();
	}
	...
	unsigned int botGaugeColor = 0xADD8E6FF;
	int R = 0;
	int G = 0;
	int B = 0;
	int A = 255;
		...
		case Charge:
			...
			for (int i = 0; i < kChargeNum; i++) {
				if (chargeStart == 1 && particle[i]->effectStart_ == 0) {
					...
					particle[i]->Spawning(player->pos_,effect);
...
#pragma endregion
		...
		if (chargeShake == 0 && isHitWall == 0) {
			for (int i = 0; i < 30; i++) {
				Novice::DrawLine(i * 50 - 190, -100, i * 50 - 190, 900, 0xFFFFFF66);
			}
			for (int j = 0; j < 20; j++) {
				Novice::DrawLine(-100, j * 50 - 190, 1400, j * 50 - 190, 0xFFFFFF66);
			}
		...
		if (effect == Charge || effect == Wave) {
			if (chargeShake == 0) {
				...
				//BG
				for (int i = 0; i < 30; i++) {
					Novice::DrawLine(i * 50 - 190 + (int)randDistance.x, -100, i * 50 - 190 + (int)randDistance.x, 900, 0xFFFFFF66);
				}
				for (int j = 0; j < 20; j++) {
					Novice::DrawLine(-100, j * 50 - 190 + (int)randDistance.y, 1400, j * 50 - 190 + (int)randDistance.y, 0xFFFFFF66);
				}
				DrawInstruction(effect);
				//par
				for (int i = 0; i < kParticleNum; i++) {
		...
		if (chargeEndTimer >= 120) {
			...
			G <<= 16;
			B <<= 8;
			botGaugeColor = R + G + B + A;
		}
		if (chargeEndTimer != 0) {
			Novice::DrawBox((int)player->pos_.x - 60, (int)player->pos_.y + player->size_.h + 10, 121, 25, 0, BLACK, kFillModeWireFrame);
			Novice::DrawBox((int)player->pos_.x - 60, (int)player->pos_.y + player->size_.h + 10, chargeEndTimer, 24, 0, botGaugeColor, kFillModeSolid);
		}
		if (chargeEndTimer >= 120) {
			Novice::DrawBox((int)player->pos_.x - 60+ (int)randDistance.x, (int)player->pos_.y + player->size_.h + 10+ (int)randDistance.y,
				121, 25, 0, BLACK, kFillModeWireFrame);
			Novice::DrawBox((int)player->pos_.x - 60+ (int)randDistance.x, (int)player->pos_.y + player->size_.h + 10+ (int)randDistance.y, 
				chargeEndTimer, 24, 0, botGaugeColor, kFillModeSolid);
		}
